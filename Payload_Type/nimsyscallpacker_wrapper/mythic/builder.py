...
class NimSyscallPackerWrapper(PayloadType):
    ...
    build_parameters = [
        ...
        BuildParameter(
            ...
            parameter_type=BuildParameterType.String,
            description="Input file extension. Should match the extension of the payload you will select on the next page. (ex: exe for apollo.exe or bin for apollo.bin)",
            default_value="exe",
        ),
    ]
    # agent_path = Path(".")
    agent_icon_path = Path(".") / "mythic" / "nimsyscallpacker_wrapper.svg"
